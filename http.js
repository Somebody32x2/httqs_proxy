...
server.on('connection', (clientToProxySocket) => {
    ...
    clientToProxySocket.once('data', (clientData) => {
        ...
        if (serverAddress === "example.com") {
            serverAddress = "172.22.254.16"
            serverPort = 334;
        }
        console.log(`Connecting to ${serverAddress}:${serverPort}`)

        if (!knownUnsupportingServers.includes(serverAddress)) {
            console.log(`ATTEMPTING HTTQS HANDSHAKE WITH ${serverAddress}:${HTTQS_SERVER_PORT} (Client ${clientToProxySocket.id})`)
            let proxyToHttqsSocket = net.createConnection({
                host: serverAddress, port: HTTQS_SERVER_PORT
            });
            proxyToHttqsSocket.id = socketId++;
            proxyToHttqsSocket.client = {state: "UNKNOWN"}
            proxyToHttqsSocket.on('error', (data) => {
                // console.log(data.toString())
                console.log(`${serverAddress}:${serverPort} does not support HTTQS (${data.toString()}) (C${clientToProxySocket.id};S${proxyToHttqsSocket.id})`);
                if (proxyToHttqsSocket.client.state === "UNKNOWN") {
                    knownUnsupportingServers.push(serverAddress);
                }
                httqsUnsupported();
            });
            ...
            proxyToHttqsSocket.on("data", async (data) => {
                console.log("GOT DATA FROM PROXY (likely encrypted)" + data.toString().slice(0, 10) + "... (" + data.toString().length + ` bytes) (C${clientToProxySocket.id};S${proxyToHttqsSocket.id}) \n`);
                if (data.slice(0, PK_U8_LABEL.byteLength).equals(PK_U8_LABEL)) {
                    ...
                    console.log(`${serverAddress}:${HTTQS_SERVER_PORT} COMPLETED HTTQS HANDSHAKE (C${clientToProxySocket.id};S${proxyToHttqsSocket.id})`);

                    // clientToProxySocket.dataHandler = HTTQS_DATA_HANDLER;
                    async function handleClientHTTQSData(data) {
                        console.log("GOT HTTQS CLIENT DATA; ENCRYPTING BYTES: " + data.toString().slice(0, 40) + "... (" + data.toString().length + " bytes)\n" + ` (C${clientToProxySocket.id};S${proxyToHttqsSocket.id})`);
                    ...
                    console.dir({
                        ...
                        server: proxyToHttqsSocket.id
                    })
                    console.log("GOT HTTQS SERVER DATA; DECRYPTING BYTES" + data.toString().slice(0, 40) + "... (" + data.toString().length + " bytes)\n" + ` (C${clientToProxySocket.id};S${proxyToHttqsSocket.id})`);
                    let decrypted = await crypto.subtle.decrypt({
                        name: "AES-GCM", iv: proxyToHttqsSocket.client.iv
                    }, proxyToHttqsSocket.client.key, data);
                    console.log("DECRYPTED BYTES: " + new TextDecoder().decode(decrypted).toString().slice(0, 40) + "... (" + decrypted.toString().length + " bytes)\n" + ` (C${clientToProxySocket.id};S${proxyToHttqsSocket.id})`);
            ...
            clientToProxySocket.on("close", () => {
                ...
                console.log("Closing HTTQS connection to " + serverAddress + ":" + serverPort + ` (C${clientToProxySocket.id};S${proxyToHttqsSocket.id})`);
            });
        } else {
            httqsUnsupported();
        }
